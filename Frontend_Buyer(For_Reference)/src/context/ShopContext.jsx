...
import { createContext } from "react";
// products (lowercase) is no longer needed for cart logic, only Products (uppercase)
// import { products } from "../assets/assets_new.js"; 
import { useState, useEffect } from "react";
import { toast } from "react-toastify";
import { useNavigate } from "react-router-dom";
...
const ShopContextProvider = (props) => {
  ...
  const [rentalItems, setRentalItems] = useState([]);
  const [token, setToken] = useState('');
  const [Products, setProducts] = useState([]); // <-- This is the correct data source
  const backendUrl = import.meta.env.VITE_BACKEND_URL;
  const navigate = useNavigate();
  ...
  const getCartAmount = () => {
    ...
    for (const items in cartItems) {
      // CHANGED: Use 'Products' (from state) not 'products' (from assets)
      let itemInfo = Products.find((product) => product._id === items);

      // Add a check in case product isn't loaded yet
      if (!itemInfo) continue; 

      for (const size in cartItems[items]) {
        if (cartItems[items][size] > 0) {
          // CHANGED: Use 'salePrice' (camelCase) not 'sale_price'
          totalAmount += itemInfo.salePrice * cartItems[items][size];
        }
      }
    }
  ...
  const getCartAmount_r = () => {
    ...
    for (const itemId in rentalItems) {
      ...
      // CHANGED: Use 'rentPerDay' (camelCase) not 'rent_per_day'
      totalAmount += itemInfo.rentPerDay * quantity * duration;
    }
  ...
  const value = {
    // We remove the old 'products' from the context value to avoid confusion
    currency, delivery_fee, search, setSearch, showSearch, setShowSearch, addToCart, cartItems,
    getCartCount, updateQuantity, getCartAmount, navigate, addToCart_r, rentalItems, setRentalItems,
    getCartCount_r, updateQuantity_r, getCartAmount_r, updateDuration, calculateDeposit, getProductsData, Products, setProducts, token, setToken
  }
