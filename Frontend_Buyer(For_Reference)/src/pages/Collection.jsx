...
const Collection = () => {
  // 1. Use 'Products' (from backend) instead of 'products' (from assets)
  const { search, showSearch, Products } = useContext(ShopContext); 
  const [showFilter, setShowFilter] = useState(true);
  const [filterProducts, setFilterProducts] = useState([]);
  const [category, setCategory] = useState([]);
  ...
  // Mapping of categories to their corresponding subcategories
  // As requested, this hardcoded list is unchanged for now.
  const categoryToSubCategories = {
    Sports: ["Cricket", "Football", "Gym Equipment"],
    Electronics: ["Camera", "Laptops", "Gaming Equipment"],
    Clothes:["Male","Female","Kids"]
  };
  ...
  // Apply filters based on selected categories, subcategories, availability, and search term
  const applyFilter = () => {
    // 2. Use 'Products' (uppercase P) as the base array
    let productsCopy = Products.slice(); 
    ...
    if (subCategory.length > 0) {
      // 3. Update property name: subcategory -> subCategory
      productsCopy = productsCopy.filter(item => subCategory.includes(item.subCategory)); 
    }

    // Apply Availability Filter
    if (availabilityFilter === 'sale') {
      // 4. Update property name: available_for_sale -> availableForSale
      productsCopy = productsCopy.filter(item => item.availableForSale); 
    } else if (availabilityFilter === 'rent') {
      // 5. Update property name: available_for_rent -> availableForRent
      productsCopy = productsCopy.filter(item => item.availableForRent); 
    }
  ...
  const sortProducts = () => {
    let sortedProducts = filterProducts.slice();
    switch (sortType) {
      // 6. Update all sorting property names to camelCase
      case 'low-high':
        setFilterProducts(sortedProducts.sort((a, b) => a.rentPerDay - b.rentPerDay));
        break;
      case 'high-low':
        setFilterProducts(sortedProducts.sort((a, b) => b.rentPerDay - a.rentPerDay));
        break;
      case 'high-low-sale':
        setFilterProducts(sortedProducts.sort((a, b) => b.salePrice - a.salePrice));
        break;
      case 'low-high-sale':
        setFilterProducts(sortedProducts.sort((a, b) => a.salePrice - b.salePrice));
        break;
      default:
        applyFilter();
    }
  ...
  useEffect(() => {
    applyFilter();
    // 7. Update dependency array: products -> Products
  }, [category, subCategory, search, showSearch, Products, availabilityFilter]); 
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t">
      ...
      <div className="min-w-60">
        ...
        >
          FILTERS
          <img src={assets.dropdown_icon} className={`h-3 sm:hidden ${showFilter ? 'rotate-90' : ''}`} alt="toggle filters" />
        </p>

        {/* Availability Filter (No changes needed here, logic is in state) */}
        <div className={`border border-gray-300 pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
          <p className="mb-3 text-sm font-medium">AVAILABILITY</p>
          <div className="flex flex-col gap-2 text-sm font-light text-gray-700">
            <label className="flex gap-2">
        ...
        <div className={`border border-gray-300 pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
          ...
          <div className="flex flex-col gap-2 text-sm font-light text-gray-700">
            ...
            <label className="flex gap-2">
              <input type="checkbox" className="w-3" onChange={toggleCategory} value="Clothes" checked={category.includes("Clothes")} />
              Clothes
            </label>
          </div>
        </div>

        {/* Subcategory Filter (No changes needed) */}
        {category.length > 0 && categoryToSubCategories[category[0]] && ( // Added a check in case category[0] is not in the map
          <div className={`border border-gray-300 pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
            <p className="mb-3 text-sm font-medium">SUBCATEGORIES</p>
            <div className="flex flex-col gap-2 text-sm font-light text-gray-700">
              {categoryToSubCategories[category[0]].map(sub => (
          ...
          {filterProducts.length > 0 ? (
            filterProducts.map((item, index) => (
              <ProductItem
                ...
                name={item.name}
                id={item._id}
                // 8. Update props passed to ProductItem
                salePrice={item.availableForSale ? item.salePrice : null}
                rentPrice={item.availableForRent ? item.rentPerDay : null}
                image={item.images} // Use 'images' (array)
              />
            ))
          ) : (
            <p className="text-gray-500 col-span-4 text-center">No products found.</p>
